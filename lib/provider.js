...
export function activate() {
  if (atom.config.get('build-nsl.manageDependencies') && !atom.inSpecMode()) {
...
export function provideBuilder() {
  return class nslProvider extends EventEmitter {
    ...
    isEligible() {
      ...
      const pathToJar = atom.config.get('build-nsl.pathToJar');
      ...
      try {
        ...
        // Warn only
        if (atom.inDevMode()) {
          atom.notifications.addWarning(`**${meta.name}**: No valid \`nsL.jar\` was specified in your settings`, {
            dismissable: true,
            buttons: [
              {
                text: 'Open Settings',
    ...
    settings() {
      ...
      const customArguments = atom.config.get('build-nsl.customArguments').trim().split(' ');
